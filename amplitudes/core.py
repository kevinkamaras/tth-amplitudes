...
def tthgg(t1, tbar2, h3, g4, g5, hcase):
    ...
    d1 = hp.ttg(t1, hatp15, hat5, hcase[1], ref=g4) * tthg(negp15, tbar2, h3, hat4, hcase[0], g5) / (np.vdot(p15, p15) - t1.mass**2)
    ...
    d2 = tthg(t1, negp24, h3, hat5, hcase[1], g4) * hp.ttg(hatp24, tbar2, hat4, hcase[0], ref=g5) / (np.vdot(p24, p24) - t1.mass**2)
...
def tthggg(t1, tbar2, h3, g4, g5, g6, hcase):
    ...
    d1 = hp.ttg(t1, hatp16, hat6, hcase[2], ref=g5) * tthgg(negp16, tbar2, h3, g4, hat5, hcase[:2]) / (np.vdot(p16, p16) - t1.mass**2)
    ...
    d2 = tthg(t1, hatp245, h3, hat6, hcase[2], g5) * hp.ttgg(negp245, tbar2, g4, hat5, hcase[:2]) / (np.vdot(p245, p245) - t1.mass**2)
    ...
    d3a = tthgg(t1, tbar2, h3, negp45, hat6, ['+'] + list(hcase[2])) * hp.ggg(hatp45, g4, hat5, ['-'] + list(hcase[:2])) / np.vdot(p45, p45)
    d3b = tthgg(t1, tbar2, h3, negp45, hat6, ['-'] + list(hcase[2])) * hp.ggg(hatp45, g4, hat5, ['+'] + list(hcase[:2])) / np.vdot(p45, p45)
...
def tthqqg(t1, tbar2, h3, q4, qbar5, g6, hcase):
    ...
    hatp16 = hp.massive(*(t1.vector + hat6.vector))
    negp16 = hp.massive(*(-t1.vector - hat6.vector))
    d1 = hp.ttg(t1, hatp16, hat6, hcase[2], ref=qbar5) * tthqq(negp16, tbar2, h3, q4, hat5, hcase[:2]) / (np.vdot(p16, p16) - t1.mass**2)
    ...
    d2 = tthg(t1, hatp245, h3, hat6, hcase[2], qbar5) * hp.ttqq(negp245, tbar2, q4, hat5, hcase[:2]) / (np.vdot(p245, p245) - t1.mass**2)
    ...
    d3a = tthgg(t1, tbar2, h3, negp45, hat6, ['+'] + list(hcase[2])) * hp.qqg(hatp45, q4, hat5, ['-'] + list(hcase[:2])) / np.vdot(p45, p45)
    d3b = tthgg(t1, tbar2, h3, negp45, hat6, ['-'] + list(hcase[2])) * hp.qqg(hatp45, q4, hat5, ['+'] + list(hcase[:2])) / np.vdot(p45, p45)
...
def tthgggg(t1, tbar2, h3, g4, g5, g6, g7, hcase):
    ...
    d1 = hp.ttg(t1, hatp17, hat7, hcase[3], ref=g6) * tthggg(negp17, tbar2, h3, g4, g5, hat6, hcase[:3]) / (np.vdot(p17, p17) - t1.mass**2)
    ...
    d2 = tthg(t1, hatp2456, h3, g7, hcase[3], g6) * hp.ttggg(negp2456, tbar2, g4, g5, hat6, hcase[:3]) / (np.vdot(p2456, p2456) - t1.mass**2)
    ...
    d3a = tthgg(t1, tbar2, h3, negp456, hat7, ['+'] + list(hcase[3])) * hp.gggg(hatp456, g4, g5, hat6, ['-'] + list(hcase[:3])) / np.vdot(p456, p456)
    d3b = tthgg(t1, tbar2, h3, negp456, hat7, ['-'] + list(hcase[3])) * hp.gggg(hatp456, g4, g5, hat6, ['+'] + list(hcase[:3])) / np.vdot(p456, p456)
    ...
    d4a = tthggg(t1, tbar2, h3, g4, negp56, hat7, hcase4p7a) * hp.ggg(hatp56, g5, g6, ['-'] + list(hcase[1:3])) / np.vdot(p56, p56)
    d4b = tthggg(t1, tbar2, h3, g4, negp56, hat7, hcase4p7b) * hp.ggg(hatp56, g5, g6, ['+'] + list(hcase[1:3])) / np.vdot(p56, p56)
...
def tthqqgg(t1, tbar2, h3, q4, qbar5, g6, g7, hcase):
    ...
    hatp17 = hp.massive(*(t1.vector + hat7.vector))
    negp17 = hp.massive(*(-t1.vector - hat7.vector))
    d1 = hp.ttg(t1, hatp17, hat7, hcase[3], ref=g6) * tthqqg(negp17, tbar2, h3, q4, qbar5, hat6, hcase[:3]) / (np.vdot(p17, p17) - t1.mass**2)
    ...
    d2 = tthg(t1, hatp2456, h3, g7, hcase[3], g6) * hp.ttqqg(negp2456, tbar2, q4, qbar5, hat6, hcase[:3]) / (np.vdot(p2456, p2456) - t1.mass**2)
    ...
    d3a = tthgg(t1, tbar2, h3, negp456, hat7, ['+'] + list(hcase[3])) * hp.qqgg(hatp456, q4, qbar5, hat6, ['-'] + list(hcase[:3])) / np.vdot(p456, p456)
    d3b = tthgg(t1, tbar2, h3, negp456, hat7, ['-'] + list(hcase[3])) * hp.qqgg(hatp456, q4, qbar5, hat6, ['+'] + list(hcase[:3])) / np.vdot(p456, p456)
    ...
    hcase4p7a = list(hcase[0]) + ['+'] + list(hcase[3])
    hcase4p7b = list(hcase[0]) + ['-'] + list(hcase[3])
    d4a = tthggg(t1, tbar2, h3, q4, negp56, hat7, hcase4p7a) * hp.ggg(hatp56, qbar5, g6, ['-'] + list(hcase[1:3])) / np.vdot(p56, p56)
    d4b = tthggg(t1, tbar2, h3, q4, negp56, hat7, hcase4p7b) * hp.ggg(hatp56, qbar5, g6, ['+'] + list(hcase[1:3])) / np.vdot(p56, p56)
